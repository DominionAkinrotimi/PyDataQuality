...
class DataQualityVisualizer:
    ...
    def create_missing_values_matrix(self, figsize: Tuple = None) -> plt.Figure:
        ...
        # Create heatmap
        sns.heatmap(
            missing_matrix.T,  # Transpose to have columns as y-axis
        ...
        ax.set_title(
            f"Missing Value Patterns - {self.analyzer.name}", fontsize=14, pad=20
        )
    ...
    def create_missing_values_distribution(self, figsize: Tuple = None) -> plt.Figure:
        ...
        if not columns_data:
            ...
            ax.set_title(
                f"Missing Values Distribution - {self.analyzer.name}", fontsize=14
            )
            plt.tight_layout()
            return fig
    ...
    def create_numeric_distributions(
        self, columns: List[str] = None, figsize: Tuple = None
    ) -> plt.Figure:
        """
        ...
        if columns is None:
            ...
            numeric_cols = self.analyzer.df.select_dtypes(
                include=[np.number]
            ).columns.tolist()
        else:
        ...
        plt.suptitle(
            f"Numeric Distributions - {self.analyzer.name}", fontsize=14, y=1.02
        )
        plt.tight_layout()
    ...
    def create_outlier_detection_plot(
        ...
        for idx, col in enumerate(numeric_cols):
            ...
            ax.boxplot(
                ...
                flierprops=dict(
                    marker="o", color=self.colors["critical"], markersize=4, alpha=0.5
                ),
            )
    ...
    def create_correlation_heatmap(self, figsize: Tuple = None) -> plt.Figure:
        ...
        ax.set_title(
            f"Feature Correlations - {self.analyzer.name}", fontsize=14, pad=20
        )
    ...
    def create_categorical_analysis(
        ...
        """
        if columns is None:
            cat_cols = self.analyzer.df.select_dtypes(
        ...
        for idx, col in enumerate(cat_cols):
            ...
            if len(value_counts) > max_categories:
                ...
                other_count = value_counts.tail(
                    len(value_counts) - max_categories + 1
                ).sum()
                if other_count > 0:
                    top_values = pd.concat(
                        [top_values, pd.Series([other_count], index=["Other"])]
                    )
        ...
        plt.suptitle(
            f"Categorical Analysis - {self.analyzer.name}", fontsize=14, y=1.02
        )
        plt.tight_layout()
    ...
    def create_comprehensive_report(
        ...
        if save_path:
            ...
            for name, fig in figures.items():
                filename = (
                    f"{save_path}/{self.analyzer.name.replace(' ', '_')}_{name}.png"
                )
