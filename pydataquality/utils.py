...
def sample_large_dataset(
    filepath: str, n_samples: int = 1000, chunksize: int = 100000
) -> pd.DataFrame:
    """
...
def load_rules_from_yaml(filepath: str) -> Dict[str, Any]:
    ...
    try:
        ...
        warnings.warn(
            "PyYAML not installed. Please install it to use YAML config: pip install PyYAML"
        )
        return {}
    except Exception as e:
    ...
    for key, value in thresholds.items():
        if key in validated:
            if 0 <= value <= 1:
                ...
                warnings.warn(
                    f"Threshold {key} value {value} should be between 0 and 1. Using default."
                )
...
def detect_potential_ids(df: pd.DataFrame) -> List[Dict[str, Any]]:
    """
    ...
    for col in date_columns:
        ...
        try:
            ...
            date_stats = {
                ...
                "past_century_dates": int(
                    (valid_dates < pd.Timestamp("1900-01-01")).sum()
                ),
            ...
            if pd.api.types.is_string_dtype(df[col]):
                ...
                if len(unique_formats) > 1:
                    date_stats["format_issues"].append(
                        f"Multiple date separators detected: {unique_formats}"
                    )
