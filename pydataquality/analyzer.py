...
class DataQualityAnalyzer:
        ...
        stats.stats.update(
            {
                ...
                "most_common_count": (
                    int(categorical_data.value_counts().iloc[0])
                    if len(categorical_data) > 0
                    else 0
                ),
                "most_common_percentage": (
                    float(
                        categorical_data.value_counts().iloc[0]
    ...
    def _check_numeric_issues(self, col_data: pd.Series, stats: ColumnStats):
                ...
                if len(outliers) > 0:
                    ...
                    self.issues.append(
                        QualityIssue(
                            ...
                            issue_type="outliers",
                            severity=(
                                "warning" if outlier_percentage < 10 else "critical"
                            ),
                            message=message,
        ...
        if (
            "skew" in stats.stats
            and abs(stats.stats["skew"]) > self.config["skew_threshold"]
        ):
            self.issues.append(
                QualityIssue(
                    column=stats.name,
    ...
    def _check_categorical_issues(self, col_data: pd.Series, stats: ColumnStats):
        ...
        if "value_counts" in stats.stats:
            ...
            # Group original values by their lowercase stripped forms
            normalized_groups = {}
            for value in value_counts.keys():
                if isinstance(value, str):
                    ...
                    if norm_val in normalized_groups:
                        normalized_groups[norm_val].append(value)
                    else:
                        normalized_groups[norm_val] = [value]

            # Find groups where a single normalized key maps to multiple original keys
            inconsistent_groups = {
                norm: origs
                for norm, origs in normalized_groups.items()
                if len(origs) > 1
            }
            ...
            if len(inconsistent_groups) > 0:
                affected_count = sum(
                    value_counts[orig]
                    for origs in inconsistent_groups.values()
                    for orig in origs
                )
                self.issues.append(
                    QualityIssue(
                        ...
                        affected_percentage=affected_count / stats.total_count * 100,
                        details={
                            "unique_normalized_values": len(normalized_groups),
                            "examples": [
                                f"Normalized '{k}': {origs}"
                                for k, origs in inconsistent_groups.items()
                            ][:5],
                        },
                    )
                )
        ...
        if "future_dates" in stats.stats and stats.stats["future_dates"] > 0:
            self.issues.append(
                QualityIssue(
                    ...
                    affected_percentage=stats.stats["future_dates"]
                    / stats.total_count
                    * 100,
                )
            )
    ...
    def _get_missing_data_overview(self) -> Dict:
        ...
        return {
            ...
            "total_missing_cells": sum(
                stats.missing_count for stats in self.column_stats.values()
            ),
            "total_missing_percentage": (
                sum(stats.missing_count for stats in self.column_stats.values())
                / (self.dataset_stats["rows"] * self.dataset_stats["columns"])
                * 100
                if self.dataset_stats["rows"] * self.dataset_stats["columns"] > 0
                else 0
            ),
    ...
    def get_problematic_rows(
        self, column: str, issue_type: str = "all"
    ) -> pd.DataFrame:
        """
        ...
        if issue_type in ["outliers", "all"] and pd.api.types.is_numeric_dtype(
            col_data
        ):
            try:
                numeric_data = col_data.dropna()
